...
class BiLSTM_F:
    ...
    def create_model(self):
        ...
        decoder_input = Input(shape=(None,self.tagvocabsize))
        ...
        decoder_output, _, _ = decoder(decoder_input, initial_state=encoder_com_states)
        ...
        # Src Attention
        src_attn = dot([decoder_output, encoder_src_output], axes=[2,2])
        src_attn = Activation('softmax')(src_attn)
        src_context = dot([src_attn, encoder_src_output], axes=[2,1])

        # combine attention and decoder input vectors
        context = concatenate([com_context, decoder_output, src_context])
        ...
        # Comment Attention
        cattn = dot([decoder_output, encoder_com_in], axes=[2, 2])
        cattn = Activation('softmax')(cattn)
        ccontext = dot([cattn, encoder_com_in], axes=[2,1])

        # Src Attention
        sattn = dot([decoder_output, encoder_src_in], axes=[2,2])
        sattn = Activation('softmax')(sattn)
        scontext = dot([sattn, encoder_src_in], axes=[2,1])

        new_context = concatenate([ccontext, decoder_output, scontext])
