...
class BiLSTM:
    ...
    def create_model(self):
        ...
        encoder_embedding = Embedding(output_dim=300, input_dim=self.comvocabsize, mask_zero=False)(encoder_inputs)
        encoder = Bidirectional(CuDNNLSTM(lstm_units, return_state=True))
        ...
        # Tag input
        decoder_inputs = Input(shape=(None,self.tagvocabsize))
        #decoder_embedding = Embedding(output_dim=2, input_dim=self.tagvocabsize, mask_zero=False)(decoder_inputs)
        decoder_lstm = CuDNNLSTM(lstm_units*2, return_sequences=True, return_state=True)
